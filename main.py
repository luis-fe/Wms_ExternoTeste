...
def get_usuariosRestricao():
    usuarios = Usuario.PesquisarSenha()
...
def update_usuario(codigo):
    ...
    nome_ant, funcao_ant, situacao_ant = Usuario.PesquisarUsuariosCodigo(codigo)
    if 'funcao' in data:
    ...
    Usuario.AtualizarInformacoes(nome_novo, nova_funcao, situacao_novo, codigo)
...
def criar_usuario():
    ...
    c, n, f = Usuario.PesquisarUsuariosCodigo(codigo)
    if c != 0:
        return jsonify({'message': f'Novo usuário:{codigo}- {nome} ja existe'}), 201
    else:
        Usuario.InserirUsuario(codigo, funcao, nome, senha, situacao)
