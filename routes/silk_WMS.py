...
def get_PesquisaEndereco():
    ...
    # Crie um dicionário para agrupar os valores por coluna
    data_dict = {column_name: [] for column_name in column_names}

    for index, row in resultados.iterrows():
        for column_name in column_names:
            data_dict[column_name].append(row[column_name])
...
def delete_endpoint():
    ...
    if resultado == True:
        return jsonify({'Mesagem':f'endereco: {endereco}, produto {produto}  EXCLUIDOS NO CADASTRO DE SILK'}), 200
    else:
