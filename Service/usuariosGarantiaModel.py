...
def PesquisarUsuariosPortal():
    conn = ConexaoPostgreMPL.conexao()
    cursor = conn.cursor()
    cursor.execute('select codigo, nome, funcao, situacao from "Reposicao".off.usuariosportal c order by nome asc')
...
def PesquisarSenha():
    conn = ConexaoPostgreMPL.conexao()
    cursor = conn.cursor()
    cursor.execute('select codigo, nome, senha from "Reposicao".off.usuariosportal c')
    usuarios = cursor.fetchall()
    cursor.close()
    conn.close()
    return usuarios
...
def AtualizarInformacoes(novo_nome, nova_funcao, nova_situacao,  codigo):
    conn = ConexaoPostgreMPL.conexao()
    cursor = conn.cursor()
    cursor.execute('UPDATE "Reposicao".off.usuariosportal SET nome=%s, funcao=%s, situacao= %s WHERE codigo=%s',(novo_nome,nova_funcao,nova_situacao, codigo))
    conn.commit()
    cursor.close()
    conn.close()
    return novo_nome
...
def InserirUsuario(codigo, funcao, nome, senha, situacao, empresa):
    conn = ConexaoPostgreMPL.conexao()
    cursor = conn.cursor()
    cursor.execute('INSERT INTO "Reposicao".off.usuariosportal (codigo, funcao, nome, senha, situacao, empresa) '
...
def ConsultaUsuarioSenha(codigo, senha):
    ...
    query = 'SELECT COUNT(*) FROM "Reposicao".off.usuariosportal WHERE codigo = %s AND senha = %s '
