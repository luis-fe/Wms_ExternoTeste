...
def EtiquetaPrateleira(saida_pdf,endereco, rua,modulo,posicao):
    # Configurações das etiquetas e colunas
    label_width = 7.5 * cm
    label_height = 1.8 * cm
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as temp_qr_file:
        ...
        title = modulo
        c.setFont("Helvetica-Bold", 21)
        c.drawString(1.4 * cm, 0.75 * cm, title)

        c.setFont("Helvetica-Bold", 9)
        c.drawString(1.8 * cm, 1.5 * cm, 'Quadra.')

        title = posicao
        c.setFont("Helvetica-Bold", 21)
        c.drawString(2.9 * cm, 0.75 * cm, title)

        c.setFont("Helvetica-Bold", 9)
        c.drawString(2.9 * cm, 1.5 * cm, 'Posicao.')
